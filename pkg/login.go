...
func (c *Creds) Login() (Client, error) {
	...
	if err != nil {
		return &astraops.APIClient{}, fmt.Errorf("unable to read conf dir with error '%v'", err)
	}
	hasToken, err := confFile.HasToken()
	if err != nil {
		return &astraops.APIClient{}, fmt.Errorf("unable to read token file '%v' with error '%v'", confFile.TokenPath, err)
	}
	var client *astraops.APIClient
	if hasToken {
		token, err := ReadToken(confFile.TokenPath)
		if err != nil {
			return &astraops.APIClient{}, fmt.Errorf("found token at '%v' but unable to read token with error '%v'", confFile.TokenPath, err)
		}
		return astra.AuthenticateToken(token, env.Verbose), nil
	}
	hasSa, err := confFile.HasServiceAccount()
	if err != nil {
		return &astraops.APIClient{}, fmt.Errorf("unable to read service account file '%v' with error '%v'", confFile.SaPath, err)
	}
	if !hasSa {
		return &astraops.APIClient{}, fmt.Errorf("unable to access any file for directory `%v`, run astra-cli login first", confDir)
	}
	clientInfo, err := ReadLogin(confFile.SaPath)
	if err != nil {
		return &astraops.APIClient{}, err
	}
	client, err = astra.Authenticate(clientInfo, env.Verbose)
	if err != nil {
		return &astraops.APIClient{}, fmt.Errorf("authenticate failed with error %v", err)
	}
	return client, nil
}
